...
function get_que(index){
    return document.getElementsByClassName("q-content")[index - 1];
}

function get_que_num(){
    return document.getElementsByClassName("q-content").length;
}
...
function set_opoint_text(elem, index, text){
...
function set_opoints_text(s, t, text){
    ...
    for(var i = s;i <= t;i++){
        var x = get_que(i);
        for(var j = 1;j <= 4;j++){
            set_opoint_text(x, j, String.fromCharCode("A".charCodeAt() + j - 1) + str_reslove(text, i - s + 1, j));
        }
    }
    restart();
}
